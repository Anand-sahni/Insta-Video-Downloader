const insta = (e) => document.querySelector(e);
const render = insta(".result");
  ...
  if (url) {
    fetch(url)
      ...
      .then((r) => {
        ...
        let w = setTimeout(() => {
          let v = insta('meta[property="og:video"]');
          if (v) {
            createVideo(v);
          } else {
            let img = insta('meta[property="og:image"]');
            if (img) {
              createImg(img);
            } else {
